  ...
  });
{
  ...
  }
  {
    ...
    // Wenjun comment: we change the definition of changes of output so that it is a array with three objects : plot2D, plot3D and report
  // instead of the original defintion which has a strange name  'plot2DOrPlot3DOrReport'.
  // For test purpose

  /* 
  convert a string (xml) to jsonx object
 
 jsonix = $.jsx.unmarshaller.unmarshalString('<sedML xmlns="http://sed-ml.org/">
  <listOfOutputs>
	...
	convert the jsonx object back to string (xml)
	  $.jsx.marshaller.marshalString(jsonix)
 	
*/	
  
    {
    org.sed_ml.ListOfOutputs.baseTypeInfo = org.sed_ml.SEDBase;
    org.sed_ml.ListOfOutputs.properties = [new Jsonix.Model.ElementPropertyInfo({
          name: 'plot2D',
