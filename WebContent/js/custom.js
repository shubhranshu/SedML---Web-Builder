...
var xHeader = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>';
var xDoc;
var nodeUid = 0;
...
//			val =  this.childNodes[1].firstChild.value;
		...
		$(uid + ' > .options > .element').each(function(i){
			...
			if(coll){
			// to check whether the cNode[name] has been created, if not then create it as a new array
 ...
 //	  to convert a html node to an xml document
     ...
     this.htmlToXml = function() {
	    	 ...
	    	 string =  $.jsx.marshaller.marshalString($.jsx.rootNode);
			 return string;
    }

	this.attachProperties = function(uid, cNode){
		$(uid + ' > .attribute').each(function(kk){
			attrName = this.childNodes[0].textContent;
			val = $.jsx.convertToDesiredType(node, attrName, this.childNodes[1].firstChild.value);
			if(val)
				cNode[attrName] = val;
...
function toXml(){
	...
	_xml = xHeader + _xml;
	//_xml = xHeader + _xml;
	//_xml = stringToXml(_xml);
	//xDocDisplay = x2xProcessor.transformToDocument(_xml);
	//xString = (new XMLSerializer()).serializeToString(xDocDisplay); 
	$('#ModelXML').val(_xml);
}

function initXSLProcessor(){
	x2xProcessor = new XSLTProcessor();
	var x_xsl = document.implementation.createDocument("", "xsl", null);
	x_xsl.load("http://localhost:8080/SedML-WebBuilder/xsl/sedml-xsl2.xsl.xml");
	x2xProcessor.importStylesheet(x_xsl);
}

function buildCellMLModel(){
